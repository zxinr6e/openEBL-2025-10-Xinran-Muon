...
def single_bus_ring_res(
    ...
    for i in range(len(sweep_gap)):
        ...
        # x-offset
        xo = 2*r - cell_ebeam_gc.bbox().width()*dbu - 14
        if xo > 0:
            x += xo
        
        # Grating couplers, Ports 0, 1 (from the bottom up)
        instGCs = []
        for i in range(0,2):
            t = Trans(Trans.R0, to_itype(x,dbu), i*127/dbu + y_row)
            instGCs.append( cell.insert(CellInstArray(cell_ebeam_gc.cell_index(), t)) )
        
        # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
        t = Trans(Trans.R90, to_itype(x,dbu), to_itype(GC_pitch*1,dbu) + y_row)
        text = Text ("opt_in_%s_1550_device_%s_RingSingleBus%sr%sg%s" % (pol.upper(), designer_name, pol.upper(),r,int(round(g*1000))), t)
        text.halign = 1
        cell.shapes(TextLayerN).insert(text).text_size = 5/dbu
                  
        # Ring resonator from directional coupler + waveguide PCells
        cell_dc = ly.create_cell("ebeam_dc_halfring_straight", "EBeam", { "r": r, "w": wg_width, "g": g, "bustype": 0 } )
        y_ring = GC_pitch*1/2
        # first directional coupler
        t1 = Trans(Trans.R90, to_itype(x+wg_bend_radius, dbu), to_itype(y_ring, dbu) + y_row)
        inst_dc1 = cell.insert(CellInstArray(cell_dc.cell_index(), t1))
        # add waveguide, snapped to the first one
        wg = ly.create_cell("Waveguide_Arc", "EBeam_Beta", { "radius": r, "wg_width": wg_width, "start_angle": 0, "stop_angle": 180 } )
        inst_wg = connect_cell(inst_dc1, 'pin2', wg, 'pin1')
        ...
        inst_wg1 = connect_pins_with_waveguide(instGCs[0], 'opt1', inst_dc1, 'pin1', waveguide_type=waveguide_type)
